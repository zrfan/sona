...
import com.tencent.angel.sona.ml.param.shared.{HasFeaturesCol, HasLabelCol, HasPredictionCol, HasWeightCol}
...
M <: PredictionModel[FeaturesType, M]]
  ...
  override def fit(dataset: Dataset[_]): M = {
    ...
    val labelCasted = DatasetUtil.withColumn(dataset, $(labelCol), col($(labelCol)).cast(DoubleType), labelMeta)
    ...
    val casted = this match {
      case p: HasWeightCol =>
        if (isDefined(p.weightCol) && $(p.weightCol).nonEmpty) {
          ...
          DatasetUtil.withColumn(labelCasted, $(p.weightCol), col($(p.weightCol)).cast(DoubleType), weightMeta)
        } else {
          labelCasted
        }
      case _ => labelCasted
  ...
  protected def extractLabeledPoints(dataset: Dataset[_]): RDD[LabeledPoint] = {
    dataset.select(col($(labelCol)), col($(featuresCol))).rdd.map {
      case Row(label: Double, features: Vector) => feature.LabeledPoint(label, features)
    }
  }
}
